...
from drf_comments.compat import apply_markdown, pygments_highlight
from drf_comments.renderers import HTMLFormRenderer
from drf_comments.utils.urls import replace_query_param
...
def optional_login(request):
    ...
    """
    try:
        login_url = reverse('drf_comments:login')
...
def optional_logout(request, user, csrf_token):
    ...
    """
    try:
        logout_url = reverse('drf_comments:logout')
    except NoReverseMatch:
...
def format_value(value):
    ...
    if value is None or isinstance(value, bool):
        ...
        if any(isinstance(item, (list, dict)) for item in value):
            template = loader.get_template('drf_comments/admin/list_value.html')
        else:
            template = loader.get_template('drf_comments/admin/simple_list_value.html')
        context = {'value': value}
        return template.render(context)
    elif isinstance(value, dict):
        template = loader.get_template('drf_comments/admin/dict_value.html')
        context = {'value': value}
        return template.render(context)
